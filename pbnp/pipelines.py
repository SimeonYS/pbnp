...
class PbnpPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('pbnp.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `pbnp`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM pbnp WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO pbnp (date, title, link, content)"
